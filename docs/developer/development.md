...
Databricks Connect must be equal to or older than the target runtime, so
Connect 17 supports DBR 17, 18, and 19.
...
compatibility testing. `uv.lock` keeps all environments reproducible.
...
Ruff and Pyright configuration lives in `pyproject.toml`.
...
Tests use Databricks Connect and `databricks-labs-pytester`. The examples cover:
